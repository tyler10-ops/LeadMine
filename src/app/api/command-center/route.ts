...
export async function GET() {
  try {
    ...
    // Leads are stored under the realtor id OR the client id depending on the
    // account type — resolve both and aggregate across all of them (matches
    // /api/leads/property). Service client bypasses RLS; we scope manually.
    const supabase = createServiceClient();
    const [{ data: client }, { data: realtor }] = await Promise.all([
      supabase.from("clients").select("id").eq("user_id", user.id).maybeSingle(),
      supabase.from("realtors").select("id").eq("user_id", user.id).maybeSingle(),
    ]);
    const allowedIds = [client?.id, realtor?.id, user.id].filter(Boolean) as string[];
    const has = allowedIds.length > 0;

    // Stage counts
    const stageCounts = { new: 0, contacted: 0, qualified: 0, booked: 0, dead: 0 };
    if (has) {
      const { data: stageRows } = await supabase
        .from("leads").select("stage").in("client_id", allowedIds);
    ...
    // Tier counts (gem grades)
    const tierCounts = { diamond: 0, hot: 0, warm: 0, cold: 0, total: 0 };
    if (has) {
      const { data: tierRows } = await supabase
        .from("leads").select("gem_grade").in("client_id", allowedIds);
    ...
    if (has) {
      const { data: lastRun } = await supabase
        ...
        .in("client_id", allowedIds).eq("event_type", "mine_completed")
    ...
    if (has) {
      ...
      const { count: drafts } = await supabase
        ...
        .in("realtor_id", allowedIds).gte("created_at", startOfMonth.toISOString());
      const { count: approved } = await supabase
        .from("follow_up_activities").select("id", { count: "exact", head: true })
        .in("realtor_id", allowedIds).eq("status", "sent").gte("created_at", startOfMonth.toISOString());
    ...
    if (has) {
      const { count } = await supabase
        ...
        .in("client_id", allowedIds).eq("stage", "new");
